export const cards = [
  {
    icon: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRFGmzvRt2QXfnlr3DHz6C73PzvYQ5RPJyaxxG_2dvMmzIJjaJ66y4sTp_4F4bvE78jsIc&usqp=CAU',
    backgroundColor: '#fff',
    content: 'https://thumbs.gfycat.com/PastelHalfHerring-size_restricted.gif',
  },
  {
    icon: 'https://yt3.googleusercontent.com/18xKdR1V_SHCF7CtFQ_TrMEtAB5XeixajzOwnViaCIa9gV7VXUg72LUxOHYDU8kn5mOlxn1bTRI=s900-c-k-c0x00ffffff-no-rj',
    backgroundColor: '#fff',
    content:
      'https://thumbs.gfycat.com/UnsungSorrowfulAfricanpiedkingfisher-size_restricted.gif',
  },
  {
    icon: 'https://www.lg.com/lg5-common-gp/images/common/share/share-default.jpg',
    backgroundColor: '#fff',
    content: 'https://i.gifer.com/14M1.gif',
  },
  {
    icon: 'https://cdn.worldvectorlogo.com/logos/vasco-futebol-clube-de-sapiranga-rs.svg',
    backgroundColor: '#fff',
    content: 'https://i.gifer.com/14M1.gif',
  },
  {
    icon: 'https://styles.redditmedia.com/t5_4mbrq5/styles/communityIcon_4b3ni8g4krf81.png?width=256&s=9b56100040472f74b807d4e6c1e0bc20d35e1b39',
    backgroundColor: '#fff',
  },
  {
    icon: 'data:image/jpeg;base64,',
    backgroundColor: '#fff',
  },
  {
    icon: 'https://portalpadrao.ufma.br/site/noticias/tv-ufma-estreia-nova-programacao-com-destaque-para-o-telejornal/tv-ufma.jpg/@@images/b069e5a4-36f0-4b4f-8dd1-55a41c66e1f4.jpeg',
    backgroundColor: '#fff',
  },
  {
    icon: 'https://yt3.googleusercontent.com/ytc/AOPolaQg3KSoXjt1tP5UWthZywstBoMEPBuufhcSguh0_g=s900-c-k-c0x00ffffff-no-rj',
    backgroundColor: '#fff',
  },
  {
    icon: 'https://yt3.googleusercontent.com/ytc/AOPolaSEs3OgpEVC0aMbRww_6SRCN4sOXItd851tdRtS=s900-c-k-c0x00ffffff-no-rj',
    backgroundColor: '#fff',
  },
];

export const streaming = [
  {
    icon: 'https://assets.nintendo.com/image/upload/c_fill,w_1200/q_auto:best/f_auto/dpr_2.0/ncom/software/switch/70010000063714/ea2b7333df08b27cb7313d6586838d761755fc8a17b91b17a456c966dc1605cc',
    backgroundColor: '#fff',
    content:
      "https://www.youtube.com/embed/wIJODMsYbkc?controls=0&showinfo=0&rel=0&autoplay=1&loop=1&mute=1'",
  },
  {
    icon: 'https://hbomax-images.warnermediacdn.com/2020-05/square%20social%20logo%20400%20x%20400_0.png',
    backgroundColor: '#fff',
    content:
      "https://www.youtube.com/embed/wIJODMsYbkc?controls=0&showinfo=0&rel=0&autoplay=1&loop=1&mute=1'",
  },
  {
    icon: 'https://s2-techtudo.glbimg.com/m5VjqU_ldJcwBQ2GOqIxPNUD9a8=/0x0:620x465/984x0/smart/filters:strip_icc()/i.s3.glbimg.com/v1/AUTH_08fbf48bc0524877943fe86e43087e7a/internal_photos/bs/2021/Z/v/vL68NTRMyL2ttQPB8voQ/2012-03-28-foto-1.jpg',
    backgroundColor: '#fff',
    content:
      "https://www.youtube.com/embed/wIJODMsYbkc?controls=0&showinfo=0&rel=0&autoplay=1&loop=1&mute=1'",
  },
  {
    icon: 'data:image/png;base64,',
    backgroundColor: '#fff',
    content:
      "https://www.youtube.com/embed/wIJODMsYbkc?controls=0&showinfo=0&rel=0&autoplay=1&loop=1&mute=1'",
  },
  {
    icon: 'https://static-assets.bamgrid.com/product/disneyplus/images/share-default.14fadd993578b9916f855cebafb71e62.png',
    backgroundColor: '#fff',
    content:
      "https://www.youtube.com/embed/wIJODMsYbkc?controls=0&showinfo=0&rel=0&autoplay=1&loop=1&mute=1'",
  },
  {
    icon: 'https://m.media-amazon.com/images/I/513naL3U+8L.png',
    backgroundColor: '#fff',
    content: 'https://m.media-amazon.com/images/I/513naL3U+8L.png',
  },
  {
    icon: 'https://m.media-amazon.com/images/G/01/support_images/GUID-A5E374A8-16DA-4B39-8E3F-3F3B34E831FB=2=pt-BR=Normal.png',
    backgroundColor: '#fff',
    content: 'https://m.media-amazon.com/images/I/513naL3U+8L.png',
  },

  {
    icon: 'https://assets.nintendo.com/image/upload/c_fill,w_1200/q_auto:best/f_auto/dpr_2.0/ncom/software/switch/70010000063714/ea2b7333df08b27cb7313d6586838d761755fc8a17b91b17a456c966dc1605cc',
    backgroundColor: '#fff',
    content:
      "https://www.youtube.com/embed/wIJODMsYbkc?controls=0&showinfo=0&rel=0&autoplay=1&loop=1&mute=1'",
  },
  {
    icon: 'https://static-assets.bamgrid.com/product/disneyplus/images/share-default.14fadd993578b9916f855cebafb71e62.png',
    backgroundColor: '#fff',
    content:
      "https://www.youtube.com/embed/wIJODMsYbkc?controls=0&showinfo=0&rel=0&autoplay=1&loop=1&mute=1'",
  },
];
