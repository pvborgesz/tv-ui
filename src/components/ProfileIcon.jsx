...
export default function ProfileIcon({ icon, name, create }) {
  ...
  return (
    <Link
      className="flex flex-col items-center gap-10 cursor-pointer hover:bg-zinc-500 p-20 rounded-sm border-2 bg-gradient-to-r from-sky-950 to-zinc-800"
      to={href}
    >
      <div className="flex flex-row gap-2 hover:bg-zinc-500 p-1 rounded-sm flex-1">
        <div className="flex flex-col items-center gap-10 cursor-pointer hover:bg-zinc-500 p-2 rounded-sm">
          ...
          />
          <h4 className="text-2xl mt-1.5 font-bold mb-1.5">{name}</h4>
        </div>
      </div>
    </Link>
  );
